...
本 repo 的 pipeline（`.github/workflows/`：`ci.yml`、`deploy-frontend.yml`、`deploy-backend.yml`）會：PR 跑品質閘門；`main` 通過後部署到 **PROD**（前端 S3／CloudFront、後端 EC2＋pm2）。建議用 **GitHub OIDC → AWS IAM Role**，避免長期 Access Key 放在 Secrets。
...
| `deploy-frontend.yml` | `push` → `main`（path filter；建議 branch protection 先過 CI） | OIDC／Access Key → `aws s3 sync` `web/frontend/dist` → CloudFront invalidate |
| `deploy-backend.yml` | `push` → `main`（path filter；建議 branch protection 先過 CI） | SSH → 上傳 → 遠端 `npm ci --omit=dev` → `pm2 reload` |
...
- Workflow 已建立於 `.github/workflows/`（`ci.yml`、`deploy-frontend.yml`、`deploy-backend.yml`）；首次上線前仍須在 GitHub 填 Variables／Secrets，並完成 OIDC（或 Access Key）與 branch protection
- Deploy 與 CI 分檔：靠 PR 必過 CI（branch protection）閘門；deploy job 內再 build（對齊 `ops/deploy-*.sh`），不跨 workflow 使用 `needs`
- **不維護 `develop`／DEV AWS**（成本考量）；品質靠本機驗收 + PR CI
- Actions 失敗不會自動還原 S3／EC2 上一版
- `npm audit`、Request ID／body limit 等硬化可另開小 commit
